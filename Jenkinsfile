...
pipeline {
    parameters {
        string(name: 'ref', defaultValue: '', description: 'Used to automatically tag built docker images based on git tag. Leave blank for manual builds.')
    }
    options {
...
//         }
        stage('Docker Build and Push') {
            ...
            steps {
                container('aws-cli') {
                    script {
                        withEnv(["PASSWORD=`aws ecr get-login-password --region us-west-2`"]) {
                            try {
                                script {
                                ...
                                then
                                    ...
                                    # dev
                                    docker tag "${env.CONTAINER_REGISTRY}:${env.BUILD_ID}" "${env.CONTAINER_REGISTRY}:${env.BUILD_ID}-dev"
                                    docker push "${env.CONTAINER_REGISTRY}:${env.BUILD_ID}-dev"
                                fi
                                """
                            }
                            catch(Exception e) {
